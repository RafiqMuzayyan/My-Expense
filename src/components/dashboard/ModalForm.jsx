...
const ModalForm = () => {
    ...
    return (
      <>  
        <button 
          className='flex items-center px-3 py-1 bg-white shadow rounded-xl gap-3 text-sm  ml-8 cursor-pointer'
          type='button'
          ...
          <div className='w-full h-full fixed top-0 left-0 flex justify-center items-center bg-black/50 backdrop-blur-sm z-50'>
              <div className='bg-white rounded-2xl p-6 w-full max-w-md mx-4 shadow-2xl'>
                  ...
                  <div className='flex gap-3 mt-6'>
                      <button 
                          type='button'
                          className='flex-1 px-4 py-2.5 border-2 border-black/10 rounded-lg font-medium text-black/80 hover:bg-black/5 transition-colors cursor-pointer'
                          onClick={() => setIsOpen(prev => !prev)}
                      >
                          Cancel
                      </button>
                      <button 
                          type='submit'
                          className='flex-1 px-4 py-2.5 bg-secondary rounded-lg font-medium text-white hover:bg-secondary/90 transition-colors shadow-lg shadow-secondary/30 cursor-pointer'
                      >
                          Add Transaction
                      </button>
                  </div>
